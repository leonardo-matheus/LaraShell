...
pub fn deserialize_config(path: &Path, warn_pruned: bool) -> Result<Value> {
    ...
    if (extension == "yaml" || extension == "yml") && !contents.trim().is_empty() {
        warn!(
            "YAML config {path:?} is deprecated, please migrate to TOML using `larashell migrate`"
        );
...
// TODO: Merge back with `load_imports` once `larashell migrate` is dropped.
//
...
pub fn installed_config(suffix: &str) -> Option<PathBuf> {
    let file_name = format!("larashell.{suffix}");

    // Try using XDG location by default.
    xdg::BaseDirectories::with_prefix("larashell")
        .ok()
        ...
        .or_else(|| {
            if let Ok(home) = env::var("HOME") {
                // Fallback path: $HOME/.config/larashell/larashell.toml.
                let fallback = PathBuf::from(&home).join(".config/larashell").join(&file_name);
                if fallback.exists() {
                    return Some(fallback);
                }
                // Fallback path: $HOME/.larashell.toml.
...
pub fn installed_config(suffix: &str) -> Option<PathBuf> {
    let file_name = format!("larashell.{suffix}");
    dirs::config_dir().map(|path| path.join("larashell").join(file_name)).filter(|new| new.exists())
}
