...
use larashell_terminal::tty::Options as PtyOptions;
...
/// CLI options for the main LaraShell executable.
...
pub struct Options {
    ...
    /// X11 window ID to embed LaraShell within (decimal or hexadecimal with "0x" prefix).
    #[clap(long)]
    ...
    /// Specify alternative configuration file [default: %APPDATA%\larashell\larashell.toml].
    #[cfg(windows)]
    #[clap(long, value_hint = ValueHint::FilePath)]
    pub config_file: Option<PathBuf>,

    /// Specify alternative configuration file [default: $HOME/.config/larashell/larashell.toml].
    #[cfg(target_os = "macos")]
    #[clap(long, value_hint = ValueHint::FilePath)]
    pub config_file: Option<PathBuf>,
...
pub struct WindowIdentity {
    /// Defines the window title [default: LaraShell].
    #[clap(short = 'T', short_alias('t'), long)]
    pub title: Option<String>,

    /// Defines window class/app_id on X11/Wayland [default: LaraShell].
...
/// Send a message to the LaraShell socket.
#[cfg(unix)]
#[derive(Args, Debug)]
...
pub enum SocketMessage {
    /// Create a new window in the same LaraShell process.
    CreateWindow(WindowOptions),

    /// Update the LaraShell configuration.
    Config(IpcConfig),
}
...
pub struct IpcConfig {
    ...
    #[clap(short, long, allow_hyphen_values = true, env = "LARASHELL_WINDOW_ID")]
...
mod tests {
    ...
    fn completions() {
        ...
        for (shell, file) in &[
            (Shell::Bash, "larashell.bash"),
            (Shell::Fish, "larashell.fish"),
            (Shell::Zsh, "_larashell"),
        ] {
            let mut generated = Vec::new();
            clap_complete::generate(*shell, &mut clap, "larashell", &mut generated);
        ...
        //
        // let mut file = File::create("../extra/completions/larashell.bash").unwrap();
        // clap_complete::generate(Shell::Bash, &mut clap, "larashell", &mut file);
        // let mut file = File::create("../extra/completions/larashell.fish").unwrap();
        // clap_complete::generate(Shell::Fish, &mut clap, "larashell", &mut file);
        // let mut file = File::create("../extra/completions/_larashell").unwrap();
        // clap_complete::generate(Shell::Zsh, &mut clap, "larashell", &mut file);
    }
}
