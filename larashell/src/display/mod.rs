...
use larashell_terminal::event::{EventListener, OnResize, WindowSize};
use larashell_terminal::grid::Dimensions as TermDimensions;
use larashell_terminal::index::{Column, Direction, Line, Point};
use larashell_terminal::selection::Selection;
use larashell_terminal::term::cell::Flags;
use larashell_terminal::term::{
    self, point_to_viewport, LineDamageBounds, Term, TermDamage, TermMode, MIN_COLUMNS,
    MIN_SCREEN_LINES,
};
use larashell_terminal::vte::ansi::{CursorShape, NamedColor};
...
impl Display {
    ...
    pub fn draw<T: EventListener>(
        ...
        // Add damage from larashell's UI elements overlapping terminal.
        if self.collect_damage() {
        ...
        if matches!(self.raw_window_handle, RawWindowHandle::Xcb(_) | RawWindowHandle::Xlib(_)) {
            ...
            // will block to synchronize (this is `glClear` in LaraShell), which causes a
