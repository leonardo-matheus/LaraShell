...
pub const LOG_TARGET_IPC_CONFIG: &str = "larashell_log_window_config";

/// Name for the environment variable containing the log file's path.
const LARASHELL_LOG_ENV: &str = "LARASHELL_LOG";

/// Logging target for config error messages.
pub const LOG_TARGET_CONFIG: &str = "larashell_config_derive";

/// Name for the environment variable containing extra logging targets.
///
/// The targets are semicolon separated.
const LARASHELL_EXTRA_LOG_TARGETS_ENV: &str = "LARASHELL_EXTRA_LOG_TARGETS";
...
fn extra_log_targets() -> &'static [String] {
    ...
    EXTRA_LOG_TARGETS.get_or_init(|| {
        env::var(LARASHELL_EXTRA_LOG_TARGETS_ENV)
            .map_or(Vec::new(), |targets| targets.split(';').map(ToString::to_string).collect())
    })
}

/// List of targets which will be logged by LaraShell.
const ALLOWED_TARGETS: &[&str] = &[
    LOG_TARGET_IPC_CONFIG,
    LOG_TARGET_CONFIG,
    "larashell_config_derive",
    "larashell_terminal",
    "larashell",
    "crossfont",
];
...
impl Logger {
    ...
    fn message_bar_log(&self, record: &log::Record<'_>, logfile_path: &str) {
        ...
        #[cfg(not(windows))]
        let env_var = format!("${}", LARASHELL_LOG_ENV);
        #[cfg(windows)]
        let env_var = format!("%{}%", LARASHELL_LOG_ENV);
...
impl OnDemandLogFile {
    fn new() -> Self {
        ...
        path.push(format!("LaraShell-{}.log", process::id()));

        // Set log path as an environment variable.
        env::set_var(LARASHELL_LOG_ENV, path.as_os_str());
