...
This document gives an overview over LaraShell's features beyond its terminal
emulation capabilities. To get a list with supported control sequences take a
look at [LaraShell's escape sequence support](./escape_support.md).

## Vi Mode

The vi mode allows moving around LaraShell's viewport and scrollback using the
...
Search allows you to find anything in LaraShell's scrollback buffer. You can
...
either feeds them to an external application or triggers one of LaraShell's
built-in actions.
...
LaraShell supports running multiple terminal emulators from the same LaraShell
instance. New windows can be created either by using the `CreateNewWindow`
keybinding action, or by executing the `larashell msg create-window` subcommand.
