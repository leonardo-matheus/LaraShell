# Contributing to LaraShell

Thank you for your interest in contributing to LaraShell!
...
[LaraShell issue tracker](https://github.com/larashell/larashell/issues). To reduce the number of
duplicates, please make sure to check the existing
[enhancement](https://github.com/larashell/larashell/issues?utf8=%E2%9C%93&q=is%3Aissue+label%3Aenhancement)
and
[missing feature](https://github.com/larashell/larashell/issues?utf8=%E2%9C%93&q=is%3Aissue+label%3A%22B+-+missing+feature%22)
issues.

## Bug Reports

Bug reports should be reported in the
[LaraShell issue tracker](https://github.com/larashell/larashell/issues).

If a bug was not present in a previous version of LaraShell, providing the exact commit which
introduced the regression helps out a lot.

## Patches / Pull Requests

All patches have to be sent on Github as [pull requests](https://github.com/larashell/larashell/pulls).

If you are looking for a place to start contributing to LaraShell, take a look at the
[help wanted](https://github.com/larashell/larashell/issues?q=is%3Aopen+is%3Aissue+label%3A%22help+wanted%22)
and
[easy](https://github.com/larashell/larashell/issues?q=is%3Aopen+is%3Aissue+label%3A%22D+-+easy%22)
issues.

You can find the minimum supported Rust version in LaraShell's manifest file
(`cat larashell/Cargo.toml | grep "rust-version"`). LaraShell **must** always
build with the MSRV and bumping it should be avoided if possible.

Since `larashell_terminal`'s version always tracks the next release, make sure that the version is
...
The following command can be run to test LaraShell:
...
annotation, or LaraShell's ref tests.

To record a new ref test, a release version of the patched binary should be created and run with the
`--ref-test` flag. After closing the LaraShell window, or killing it (`exit` and `^D` do not work),
...
If changes could affect throughput or latency of LaraShell, these aspects should be benchmarked to
...
LaraShell mainly uses the [vtebench](https://github.com/larashell/vtebench) tool for testing LaraShell's
performance. Instructions on how to use it can be found in its
[README](https://github.com/larashell/vtebench/blob/master/README.md).

Latency is another important factor for LaraShell. On X11, Windows, and macOS the
...
Changes compared to the latest LaraShell release which have a direct effect on the user (opposed to
...
(like commit messages). More information about LaraShell's change log format can be found
[here](https://keepachangelog.com).

### Style

All LaraShell changes are automatically verified by CI to conform to its rustfmt guidelines. If a CI
...
Unless otherwise specified, LaraShell follows the Rust compiler's style guidelines:
...
LaraShell's release process aims to provide stable and well tested releases without having to hold
...
The `larashell_terminal` crate is released in synchronization with `larashell`, keeping the `-dev`
and `-rcX` version suffix identical across the two crates. As soon as the new LaraShell stable
release is made, releases are tagged as `larashell_terminal_vX.Y.Z` and pushed to crates.io. During
a release, only the patch version is bumped on master, since there haven't been any changes since
the last release yet.

# Contact

If there are any outstanding questions about contributing to LaraShell, they can be asked on the
[LaraShell issue tracker](https://github.com/larashell/larashell/issues).

As a more immediate and direct form of communication, the LaraShell IRC channel (`#larashell` on
Libera.Chat) can be used to contact many of the LaraShell contributors.
